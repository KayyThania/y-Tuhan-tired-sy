...
function initArticleModal() {
  ...
  const articleDatabase = document.getElementById('article-database');

  if (!modal || !readMoreLinks.length || !modalContentContainer || !closeBtn || !articleDatabase) return;
  ...
  readMoreLinks.forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      const articleContent = articleDatabase.querySelector(`#${articleId}`);

      if (articleContent) {
        modalContentContainer.innerHTML = articleContent.innerHTML;
